...
class Generate
{
    ...
    public function generate_form($table = null)
    {
        ...
        $this->ci->table->set_heading('Field', 'Field Type', 'Validation ' . form_checkbox(['id' => 'all', 'name' => 'all'], 1, false), 'Show');
        $template = '';
        ...
        if($fields) {
            foreach($fields as $field) {
                ...
                if($field->type == 'varchar' || $field->type == 'text') {
                    $fields_search[] = ['field_name' => $field->name];
                }
            }
        }
        
        return ['primary_key' => $primary_key, 'field_search' => $fields_search];
    }
    ...
    private function get_field_label($table = null, $post_field)
    {
        foreach($post_field as $field => $val) {
            ...
            $label_name[] = ['label_name' => $this->set_label($field)];
    ...
    public function run($table = null, $post_field)
    {
        if($table) {
            ...
            if(! is_dir($this->output)) {
                @mkdir($this->output, DIR_WRITE_MODE, true);
            }
